...
/*!
    ...
    @discussion
        ...
        throughout the entire ARToolKit pipeline, including knowing the size of images3
...
typedef struct {
    int      xsize;		//图片尺寸
    int      ysize;
    ARdouble mat[3][4];		//投影矩阵(前三列是内参矩阵K)
    
    ARdouble dist_factor[AR_DIST_FACTOR_NUM_MAX];		//与镜头畸变相关的信息
