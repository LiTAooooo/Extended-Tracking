...
typedef struct {
    int               width;		//image的宽
    int               height;		//image的高
    int               imageNo;		//image的编号
} KpmImageInfo;

typedef struct {
    KpmImageInfo     *imageInfo;	//image数组，每个元素保存一张image信息	
    int               imageNum;		//该page中有多少个image
    int               pageNo;		//page的编号
} KpmPageInfo;

typedef struct _KpmRefData {		//每个参考特征点的数据结构
    KpmCoord2D        coord2D;		//像素坐标
    KpmCoord2D        coord3D;      	// millimetres.	//世界坐标，Z为0
    
#if BINARY_FEATURE
    FreakFeature      featureVec;	//参考特征点的描述子
    
#else	//未定义
    SurfFeature       featureVec;
#endif
    int               pageNo;		//记录这个特征点属于的page编号
    int               refImageNo;	//记录这个特征点属于对应page编号中的image编号
} KpmRefData;
 ...
 */
typedef struct {	//存储所有nft图片汇集而成的参考特征点信息
    KpmRefData       *refPoint;	//参考特征点数组	
    int               num;	//参考特征点数量
    
    KpmPageInfo      *pageInfo;		//page数组，内部每个元素保存一个page的相关信息
    int               pageNum;		//记录一共有多少张nft图片构成了这个参考特征点集
} KpmRefDataSet;	//kpm参考点数据集，用作kpmmatching的输入
...
typedef struct {
    float                     camPose[3][4];	//计算出的相机位姿
    int                       pageNo;		
    float                     error;		//误差大小，数值越大则估计越准确
    int                       inlierNum;	//匹配的特征点数量
    int                       camPoseF;		//标志位，指示是否计算出了相机的位姿 =0表示成功
    int                       skipF;
} KpmResult;
