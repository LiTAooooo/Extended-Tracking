...
static AR2HandleT *ar2CreateHandleSub( int pixFormat, int xsize, int ysize, int threadNum )
{
    ...
    arMalloc(ar2Handle, AR2HandleT, 1);		//为ar2Handle分配空间
    
    ar2Handle->pixFormat         = pixFormat;
    ar2Handle->xsize             = xsize;
    ar2Handle->ysize             = ysize;
    
#if AR2_CAPABLE_ADAPTIVE_TEMPLATE	//未执行
    ar2Handle->blurMethod        = AR2_DEFAULT_BLUR_METHOD;
    ar2Handle->blurLevel         = AR2_DEFAULT_BLUR_LEVEL;
#endif
    
    ar2Handle->searchSize        = AR2_DEFAULT_SEARCH_SIZE;		// Default radius of feature search window. 25
    ar2Handle->templateSize1     = AR2_DEFAULT_TS1;			//11
    ar2Handle->templateSize2     = AR2_DEFAULT_TS2;			//11
    ar2Handle->searchFeatureNum  = AR2_DEFAULT_SEARCH_FEATURE_NUM;	//10
    if( ar2Handle->searchFeatureNum > AR2_SEARCH_FEATURE_MAX ) {
        ar2Handle->searchFeatureNum = AR2_SEARCH_FEATURE_MAX;
    }
    
    ar2Handle->simThresh         = AR2_DEFAULT_SIM_THRESH;		//0.6
    ar2Handle->trackingThresh    = AR2_DEFAULT_TRACKING_THRESH;		//2.0
    ...
    if( threadNum > AR2_THREAD_MAX ) {					//最多8个
    ...
    for( i = 0; i < ar2Handle->threadNum; i++ ) {
        arMalloc( ar2Handle->arg[i].mfImage, ARUint8, xsize*ysize );	//为mfimage分配空间
        ar2Handle->arg[i].templ = NULL;
	
#if AR2_CAPABLE_ADAPTIVE_TEMPLATE
        ar2Handle->arg[i].templ2 = NULL;
#endif
        ar2Handle->threadHandle[i] = threadInit(i, &(ar2Handle->arg[i]), ar2Tracking2d);	//创建4个ar2Tracking2d线程，传入参数为ar2Handle->arg[i]
    }
