...
int ar2Tracking( AR2HandleT *ar2Handle, AR2SurfaceSetT *surfaceSet, ARUint8 *dataPtr, float trans[3][4], float  *err )	//ar2Handle, surfaceSet[detectedPage], gARTImage, trackingTrans, &err
{
    AR2TemplateCandidateT  *candidatePtr;
    AR2TemplateCandidateT  *cp[AR2_THREAD_MAX];
    
#if AR2_CAPABLE_ADAPTIVE_TEMPLATE		//未定义
    float                   aveBlur;
#endif
    ...
    *err = 0.0F;
    ...
    for( i = 0; i < surfaceSet->num; i++ ) {		//遍历每一个参考特征点
      
        arUtilMatMulf( (const float (*)[4])surfaceSet->trans1, (const float (*)[4])surfaceSet->surface[i].trans, ar2Handle->wtrans1[i] );	//surfaceSet->contNum=1
