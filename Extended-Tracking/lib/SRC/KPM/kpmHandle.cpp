...
static KpmHandle *kpmCreateHandleCore( ARParamLT *cparamLT, int xsize, int ysize, int poseMode, AR_PIXEL_FORMAT pixFormat )
{
    ...
    arMallocClear( kpmHandle, KpmHandle, 1 );		//分配空间

#if BINARY_FEATURE
    kpmHandle->freakMatcher             = new vision::VisualDatabaseFacade;	//定义freak匹配器
#else
    kpmHandle->ann2                     = NULL;
#endif
    ...
    kpmHandle->poseMode                = poseMode;		//六自由度KpmPose6DOF
    kpmHandle->xsize                   = xsize;
    kpmHandle->ysize                   = ysize;
    kpmHandle->pixFormat               = pixFormat;
    kpmHandle->procMode                = KpmDefaultProcMode;	//KpmProcFullSize
