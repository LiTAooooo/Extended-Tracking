...
int kpmMatching( KpmHandle *kpmHandle, ARUint8 *inImage )
{
    ...
    if (procMode == KpmProcFullSize && (kpmHandle->pixFormat == AR_PIXEL_FORMAT_MONO || kpmHandle->pixFormat == AR_PIXEL_FORMAT_420v || kpmHandle->pixFormat == AR_PIXEL_FORMAT_420f || kpmHandle->pixFormat == AR_PIXEL_FORMAT_NV21)) {
        inImageBW = inImage;
    } else {		//该条语句执行	procMode == KpmProcFullSize
        inImageBW = kpmUtilGenBWImage( inImage, kpmHandle->pixFormat, xsize, ysize, procMode, &xsize2, &ysize2 );	//1、先生成bw二值图像（灰度图）
        if( inImageBW == NULL ) return -1;
    }

#if BINARY_FEATURE
    kpmHandle->freakMatcher->query(inImageBW, xsize ,ysize);					//2、将1中的图像传入freakMatcher
    kpmHandle->inDataSet.num = (int)kpmHandle->freakMatcher->getQueryFeaturePoints().size();	//3、得到当前帧特征点的数量
    
    //ARLOGi("kpmHandle->inDataSet.num = %d\n", kpmHandle->inDataSet.num);			//测试	大概有300-500个特征点
    ...
    if( kpmHandle->inDataSet.num != 0 ) {		//如果当前帧提取出的特征点数量不为0	
        if( kpmHandle->inDataSet.coord != NULL ) free(kpmHandle->inDataSet.coord);//4、先清空特征点的存储信息
	
#if !BINARY_FEATURE	//未执行
        ...
        arMalloc( kpmHandle->inDataSet.coord, KpmCoord2D,     kpmHandle->inDataSet.num );//5、重新为特征点分配内存空间
...
#if BINARY_FEATURE
        const std::vector<vision::FeaturePoint>& points = kpmHandle->freakMatcher->getQueryFeaturePoints();	//6、提取特征点，返回一个特征点的vector
        //const std::vector<unsigned char>& descriptors = kpmHandle->freakMatcher->getQueryDescriptors();	//7、计算特征点所对应的描述子
#endif

	
        if( procMode == KpmProcFullSize ) {
            for( i = 0 ; i < kpmHandle->inDataSet.num; i++ ) {	//8、遍历每一个特征点

#if BINARY_FEATURE
                float  x = points[i].x, y = points[i].y;	//设置x、y
#else	//未执行
                float  x, y, *desc;
                ...
                if( kpmHandle->cparamLT != NULL ) {
                    ...
                    kpmHandle->inDataSet.coord[i].x = x;	//设置特征点的x、y坐标
	...
	//输入图片的特征点数量和其坐标已经设置完成
	
	
#else
        for (int pageLoop = 0; pageLoop < kpmHandle->resultNum; pageLoop++) {		//分别对每个nft标记求它们的相机位姿
            ...
            const vision::matches_t& matches = kpmHandle->freakMatcher->inliers();//得到当前帧和数据集之间的匹配结果，存放的是在两个数据集中的索引值
	    
            int matched_image_id = kpmHandle->freakMatcher->matchedId();
            if (matched_image_id < 0) continue;

            ret = kpmUtilGetPose_binary(kpmHandle->cparamLT,				//求取相机当前的位姿
                                        matches ,
...
int kpmUtilGetPose_binary(ARParamLT *cparamLT, const vision::matches_t &matchData, const std::vector<vision::Point3d<float> > &refDataSet, const std::vector<vision::FeaturePoint> &inputDataSet, float camPose[3][4], float *error)
{
    ...
    if( matchData.size() < 4 ) return -1;		//匹配的特征点需要>4
    
    arMalloc( sCoord, ICP2DCoordT, matchData.size() );	//分配空间
    arMalloc( wCoord, ICP3DCoordT, matchData.size() );
    
    for( i = 0; i < matchData.size(); i++ ) {
        sCoord[i].x = inputDataSet[matchData[i].ins].x;		//当前帧特征点像素坐标
        sCoord[i].y = inputDataSet[matchData[i].ins].y;

        wCoord[i].x = refDataSet[matchData[i].ref].x;		//数据集中对应点的坐标
