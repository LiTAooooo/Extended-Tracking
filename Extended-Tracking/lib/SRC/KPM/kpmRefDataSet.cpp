...
int kpmLoadRefDataSet( const char *filename, const char *ext, KpmRefDataSet **refDataSetPtr )	//设置了refDataSet中的特征点num、像素坐标、世界坐标、特征信息 
{
    KpmRefDataSet  *refDataSet;
    FILE           *fp;
    char            fmode[] = "rb";
    ...
    fp = kpmFopen(filename, ext, fmode);		//设置文件流
    if (!fp) {
        ARLOGe("Error loading KPM data: unable to open file '%s%s%s' for reading.\n", filename, (ext ? "." : ""), (ext ? ext : ""));
        return (-1);
    }

    arMallocClear(refDataSet, KpmRefDataSet, 1);	//分配空间
    
    if( fread(&(refDataSet->num), sizeof(int), 1, fp) != 1 ) goto bailBadRead;		//先读取参考特征点的数量
    
    //ARLOGi("refDataSet->num = %d\n", refDataSet->num);		//测试
    
    if( refDataSet->num <= 0 ) goto bailBadRead;
    
    
    arMalloc(refDataSet->refPoint, KpmRefData, refDataSet->num); // each KpmRefData = 68 floats, 3 ints = 284 bytes.	//为参考特征点分配存储空间
    
    for(i = 0; i < refDataSet->num; i++ ) {	//遍历每个参考特征点
        if( fread(  &(refDataSet->refPoint[i].coord2D), sizeof(KpmCoord2D), 1, fp) != 1 ) goto bailBadRead;		//得到参考特征点的像素坐标
        if( fread(  &(refDataSet->refPoint[i].coord3D), sizeof(KpmCoord2D), 1, fp) != 1 ) goto bailBadRead;		//得到参考特征点的实际坐标（物体坐标系，Z为0）
	
#if BINARY_FEATURE
        if( fread(  &(refDataSet->refPoint[i].featureVec), sizeof(FreakFeature), 1, fp) != 1 ) goto bailBadRead;	//读取参考特征点的特征信息
#else
        if( fread(  &(refDataSet->refPoint[i].featureVec), sizeof(SurfFeature), 1, fp) != 1 ) goto bailBadRead;
#endif
        
        if( fread(  &(refDataSet->refPoint[i].pageNo),     sizeof(int), 1, fp) != 1 ) goto bailBadRead;			//读取当前的特征点属于的page编号
       // ARLOGi("refDataSet->refPoint[%d].pageNo = %d\n",i, refDataSet->refPoint[i].pageNo);		//测试
        
        if( fread(  &(refDataSet->refPoint[i].refImageNo), sizeof(int), 1, fp) != 1 ) goto bailBadRead;			//读取当前特征点属于的image编号
        //ARLOGi("refDataSet->refPoint[%d].refImageNo = %d\n",i ,refDataSet->refPoint[i].refImageNo);		//测试
    }

    if( fread(&(refDataSet->pageNum), sizeof(int), 1, fp) != 1 ) goto bailBadRead;		//读取page总数（即有多少张Nft图片）
   // ARLOGi("refDataSet->pageNum = %d\n", refDataSet->pageNum);		//测试

    if( refDataSet->pageNum <= 0 ) {
        refDataSet->pageInfo = NULL;
        goto bailBadRead;
    }
    arMalloc(refDataSet->pageInfo, KpmPageInfo, refDataSet->pageNum);				//分配refDataSet->pageInfo数组的大小
    
    for( i = 0; i < refDataSet->pageNum; i++ ) {
        if( fread( &(refDataSet->pageInfo[i].pageNo),   sizeof(int), 1, fp) != 1 ) goto bailBadRead;
	//ARLOGi("refDataSet->pageInfo[i].pageNo = %d\n", refDataSet->pageInfo[i].pageNo);		//测试
	
        if( fread( &(refDataSet->pageInfo[i].imageNum), sizeof(int), 1, fp) != 1 ) goto bailBadRead;
	//ARLOGi("refDataSet->pageInfo[i].imageNum = %d\n", refDataSet->pageInfo[i].imageNum);		//测试
...
int kpmChangePageNoOfRefDataSet ( KpmRefDataSet *refDataSet, int oldPageNo, int newPageNo )
{
    ...
    for(int i = 0; i < refDataSet->num; i++ ) {		//遍历每个特征点
    ...
    for(int i = 0; i < refDataSet->pageNum; i++ ) {	//遍历每个page
