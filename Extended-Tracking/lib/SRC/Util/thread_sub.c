...
struct _THREAD_HANDLE_T {
    ...
    void           *arg;	//用于保存trackingInitHandle结构体
};
...
{
    pthread_mutex_lock(&(flag->mut));				//对mut加锁
    
    while(flag->startF == 0) {					//暂时还没有开始的请求				
        pthread_cond_wait(&(flag->cond1), &(flag->mut));	//此时等待信号量cond1，如果信号量到来，说明startF已经改变，此时再执行while循环进行判断
    }
    
    if( flag->startF == 1 ) {					//获得请求开始的信号
        flag->startF = 0;
        flag->busyF = 1;
